...
######################## 閾値の設定 #########################
# 青色の閾値 (HSV色空間)
lower_bound = np.array([max(color_hsv[0]-20 , 0), max(color_hsv[1] - 40, 0), max(color_hsv[2] - 40, 0)])
upper_bound = np.array([min(color_hsv[0]+20 , 179), min(color_hsv[1] + 40, 255), min(color_hsv[2] + 40, 255)])
...
for index, row in df_color.iterrows():
    ...
    if not ret:
        ...
        upper_half_box = cv2.GaussianBlur(upper_half_box, (5, 5), 0)
